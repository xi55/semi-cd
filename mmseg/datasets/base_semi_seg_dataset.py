...
class BaseSemiSegDataset(BaseDataset):
...
"""
    ...
    def load_data_list(self) -> List[dict]:
        ...
        img_path_label = self.data_prefix.get('img_path_label', None)
        img_path_unlabel = self.data_prefix.get('img_path_unlabel', None)
        ...
        if osp.isfile(self.ann_file):
            ...
            for line in lines:
                img_name = line.strip()
                data_info = dict(img_path_l=osp.join(img_path_label, img_name + self.img_suffix), \
                                 img_path_u=osp.join(img_path_unlabel, img_name + self.img_suffix))
            ...
            file_list_l = fileio.list_dir_or_file(
                    dir_path=img_path_label,
                    list_dir=False,
                    suffix=self.img_suffix,
                    recursive=True,
                    backend_args=self.backend_args)
            file_list_u = fileio.list_dir_or_file(
                    dir_path=img_path_unlabel,
                    list_dir=False,
                    suffix=self.img_suffix,
                    recursive=True,
                    backend_args=self.backend_args)
            ...
            for img_l, img_u in zip(cycle(
                    ...
                    fileio.list_dir_or_file(
                        ...
                        recursive=True,
                        backend_args=self.backend_args)):
                data_info = dict(img_path_l=[osp.join(img_path_label, img_l)],
                                 img_path_u=[osp.join(img_path_unlabel, img_u)])
                if label_path is not None:
