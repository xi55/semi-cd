...
class SemiImgLoadImageFromFile(MMCV_LoadImageFromFile):
...
@TRANSFORMS.register_module()
class SemiImgLoadAnnotations(MMCV_LoadAnnotations):
...
@TRANSFORMS.register_module()
class SemiImgSemiAnnLoadAnnotations(MMCV_LoadAnnotations):
...
@TRANSFORMS.register_module()
class SemiImgLoadLoadImageFromNDArray(SemiImgLoadImageFromFile):
...
@TRANSFORMS.register_module()
class SemiImgLoadInferencerLoader(BaseTransform):
    """Load an image pair from ``results['img']``.
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self.from_ndarray = TRANSFORMS.build(
            dict(type='SemiImgLoadLoadImageFromNDArray', **kwargs))
    ...
    def transform(self, single_input: Union[str, np.ndarray, dict]) -> dict:
        ...
        assert len(single_input) == 2, \
            'In `SemiImgLoadInferencerLoader`,' \
