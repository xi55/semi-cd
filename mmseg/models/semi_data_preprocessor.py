...
@MODELS.register_module()
class SegSemiDataPreProcessor(BaseDataPreprocessor):
    """Image pre-processor for change detection tasks.
    ...
    def forward(self, data: dict, training: bool = False) -> Dict[str, Any]:
        ...
        data_samples = data.get('data_samples', None)
        # TODO: whether normalize should be after stack_batch
        if self.channel_conversion and inputs_l[0].size(0) == 3:
            inputs_l = [_input_l[[2, 1, 0], ...] for _input_l in inputs_l]
        inputs_l = [_input_l.float() for _input_l in inputs_l]
        if self._enable_normalize:
            inputs_l = [(_input_l - self.mean) / self.std for _input_l in inputs_l]

        if self.channel_conversion and inputs_u[0].size(0) == 3:
            inputs_u = [_input_u[[2, 1, 0], ...] for _input_u in inputs_u]
        ...
        if inputs_u_s[0] is not None:
            if self.channel_conversion and inputs_u_s[0].size(0) == 3:
                inputs_u_s = [_input_u_s[[2, 1, 0], ...] for _input_u_s in inputs_u_s]
